...
getCalibFactor <- function(gdx_file, mode = "cost", calib_accuracy = 0.05) {
  ...
  data <- hist[, yrs, "crop"]
  shrLost <- (hist[getRegions(magpie), yrs, ] - setYears(hist[getRegions(magpie), 1995, ], NULL)) / setYears(hist[getRegions(magpie), 1995, ], NULL)
...
time_series_cost <- function(calib_factor) {
  ...
  # use the 2015 calib factors (last entry) for the future
  out2050 <- calib_factor[, nyears(calib_factor), ]
  out2050[out2050 < 1] <- 1
  ...
  if (all(all(calib_divergence_cost <= calib_accuracy) & all(calib_divergence_reward <= calib_accuracy)) | calibration_step == n_maxcalib) {
    ...
    if (best_calib == TRUE) {
      ...
      comment <- c(
        ...
        " unit: -",
        paste0(" note: Best calibration factors from the run"),
      ...
      write_log(setNames(calib_best_full[,seq(2000,2015,5),"cost"], NULL), "land_conversion_cost_calib_factor.cs3", "best")
      write_log(setNames(calib_best_full[,seq(2000,2015,5),"reward"], NULL), "land_conversion_reward_calib_factor.cs3", "best")
      ####
      return(TRUE)
    } else {
      return(TRUE)
